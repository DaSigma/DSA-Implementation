...
def insertion_sort_algo(my_list):
    for i in range(len(my_list)):
        ...
        while current > 0 and my_list[current - 1] > my_list[current]:
            my_list[current], my_list[current - 1] = (
                my_list[current - 1],
                my_list[current],
            )
            current -= 1
...
# l2 = [8, 10, 9, 7, 11, 0]
n = 10
random_numbers: list[int] = random.sample(range(1, 100), n)
# print('Unsorted', l, '\n')
# print(mergeSort(l))
# # l = [1,3,7,8]
# # l2 = [2,4,5,6]
...
# l = [4, 6, 1, 7, 3, 2, 5]
print(f"l {random_numbers}")
# quick_sort(l)
print(f"l after sort {insertion_sort_algo(random_numbers)}")
...
sorted_tasks.sort(
    key=cmp_to_key(lambda t1, t2: t1[1] < t2[1])
)  # Review, not 100% clear
print(f"sorted tasks {sorted_tasks}")
